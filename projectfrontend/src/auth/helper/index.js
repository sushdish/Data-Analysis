...
export const signin = user => {
  ...
  export const authenticate = (data, next) => {          //here window = chrome(i.e if chrome is define then)
    if (typeof window !== "undefined") {                  // localStrogae.setItem() is used to store data in chrome localstorage
      localStorage.setItem("jwt", JSON.stringify(data));       // date i.e jwt has to be stored in string for local storage
      next();
    }
  };
